...
public class NewDriver extends Driver {
    ...
    public boolean canLeave(Vehicle vehicle) {
        ...
        if (part instanceof Junction) {
            ...
            Road road = (Road) part;
            if (road.calcLength() / (Math.min(vehicle.getVehicleType().getAverageSpeed(), road.getMaxSpeed()) - 10)
                    - vehicle.getTimeOnCurrentPart() > 0) {
    ...
    public void stayOnCurrentPart(Vehicle vehicle) {
        if (vehicle.getCurrentRoutePart() instanceof Road) {
            ...
            System.out.println("- " + vehicle.getStatus() + road + ", time to arrive: "
                    + (road.calcLength()
                            / (Math.min(vehicle.getVehicleType().getAverageSpeed(), road.getMaxSpeed()) - 10)
